...
```{r}
locations <- read_locations("../../extdata/locations_original.csv", 
                            "../../extdata/locations.csv", 
                            "stratigraphy")
```
...
```{r}
btp <- read_btp("../../extdata/btp_original.csv", "../../extdata/btp.csv", locations)
```
...
\
In our case, we want two subjects to stand out, so we will give them a name value for the `name_spec` column.

```{r}
names     <- c(Culi = "2652", Warble = "2677")

studbook  <- read_studbook("../../extdata/studbook_original.csv", 
                           locations, btp, names = names)
```
...
write_csv(studbook_revised, "../../extdata/studbook.csv")
```
